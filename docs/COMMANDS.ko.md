...
계속 읽어야 합니다. 응답은 입력 순서대로 즉시 출력하며 창이 채워지거나 EOF가 올 때까지
기다리지 않습니다.

호환 호스트에서는 앞선 작업이 실행되는 동안 `exec --code`의 다음 컴파일을 준비합니다.
Unity API 실행 순서는 유지합니다. 파일 입력, 일반 명령, 컴파일러 경로 지정, 대상이나
연결 변경은 앞선 작업이 끝난 뒤 처리합니다. 구버전 호스트는 전송 전에 순차 경로를
선택합니다. 요청의 시간 제한에는 파이프라인 대기도 포함합니다.

보낼 코드가 이미 정해진 연속 작업에 사용하세요. 앞선 결과를 알아야 다음 코드를 만들 수
있다면 위 예제처럼 응답을 읽은 뒤 다음 요청을 보내세요. 전송한 명령의 완료 여부가
불확실해지면 세션은 확보한 응답을 출력하고 종료 코드 1로 끝나며, 아직 Unity에 전달하지
않은 대기 작업을 취소합니다. 이미 전달한 작업은 끝까지 실행될 수 있습니다. 명령을
자동으로 다시 보내지 않으므로 재요청 전 실제 효과를 확인해야 합니다. 종료 시 아직 읽지
않은 입력에는 응답이 없습니다.
